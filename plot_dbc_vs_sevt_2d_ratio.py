#!/usr/bin/env python3
"""2D distribution in (Dbc, S_EVT) plane — ratio-based Dbc, linear x-axis.

Same 4-panel structure as BDT version but with ratio Dbc and uniform Dbc bins.
"""
...
def compute_dbc_ratio(data):
    """Ratio-based Dbc (same formula as make_shape_asimov.py default)."""
    ...
    return ak.to_numpy(g_bc / (g_bc + g_qcd + g_cc + g_cs + g_qq + g_bs + g_bb + g_topbw + 1e-10))
...
def main():
    ...
    # ==================================================================
    # FIGURE: 2x2 subplots, linear x-axis, ratio Dbc
    # ==================================================================
    DBCCUT = 0.985
    ECCUT = 0.7
    dbc_bins = np.linspace(0, 1, 71)
    ...
    for idx, cat_name in enumerate(cfg.cat_order):
        ...
        ax.text(0.988, 0.94, "SR", ha='left', va='top', fontsize=12, fontweight='bold',
                color=cat_color)
        ax.text(0.988, 0.88, rf"$\epsilon$={eff_str(sr_w)}  [{count_str(sr_w)}]",
                ha='left', va='top', fontsize=10, fontfamily='monospace', color='#333333')

        # calib CR (bottom-right)
        cr_w = evt_w[mask & m_cr].sum()
        ax.text(0.988, 0.44, "calib CR", ha='left', va='center', fontsize=12, fontweight='bold',
                color=cat_color)
        ax.text(0.988, 0.38, rf"$\epsilon$={eff_str(cr_w)}  [{count_str(cr_w)}]",
                ha='left', va='center', fontsize=10, fontfamily='monospace', color='#333333')

        # Top-left
        tl_w = evt_w[mask & m_tl].sum()
        ax.text(0.012, 0.94, "passing", ha='left', va='top', fontsize=9,
                color='#999999', fontstyle='italic')
        ax.text(0.012, 0.88, rf"{eff_str(tl_w)}  [{count_str(tl_w)}]",
                ha='left', va='top', fontsize=9, fontfamily='monospace', color='#999999')

        # Bottom-left
        bl_w = evt_w[mask & m_bl].sum()
        ax.text(0.012, 0.44, "failing", ha='left', va='center', fontsize=9,
                color='#999999', fontstyle='italic')
        ax.text(0.012, 0.38, rf"{eff_str(bl_w)}  [{count_str(bl_w)}]",
    ...
    fig.supxlabel(r"$D_{bc}^{\mathrm{ratio}}$", fontsize=18, y=0.02)
    ...
    out = os.path.join(cfg.figure_path, "dbc_vs_sevt_2d_4panel_ratio.pdf")
    fig.savefig(out)
